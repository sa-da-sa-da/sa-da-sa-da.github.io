...
这个页面演示了如何实现**真正的页面间导航功能**的自定义标签页组件。与普通的标签页不同，这些标签页可以链接到不同的页面，实现网站内的便捷导航。
...
- **真实页面导航**：每个标签页链接到网站的不同页面
- **智能路由匹配**：自动检测当前页面并高亮对应的标签
- **交互友好**：悬停时显示导航图标，提供直观的视觉反馈
- **响应式设计**：适配不同屏幕尺寸
- **导航信息展示**：显示当前路由和目标路由信息

## 智能导航标签页实现

下面是具有真实导航功能的标签页组件实现代码：

<details>
<summary>查看组件实现代码</summary>
...
可以通过修改 CSS 样式来自定义导航标签的外观：

```css
/* 自定义激活标签的颜色 */
.tab-button.active {
  color: #52c41a; /* 使用绿色代替蓝色 */
...
1. 在你的页面中导入并使用 CustomTabs 组件
2. 可以根据需要自定义标签页数据和样式
3. 支持响应式设计，在不同屏幕尺寸下都能良好展示

## 配置选项

- `tabs`: 标签页数据数组
- `activeTab`: 当前激活的标签页ID
- `onTabChange`: 标签页切换时的回调函数

## 浏览器兼容性

- Chrome, Firefox, Safari, Edge 等现代浏览器
- IE 11 需要添加 polyfill 支持